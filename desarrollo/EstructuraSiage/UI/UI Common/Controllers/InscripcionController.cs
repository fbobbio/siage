...
namespace SIAGE.UI_Common.Controllers
{
    public class InscripcionController : AjaxAbmcController<InscripcionRegistrarModel, IInscripcionRules>
    {
            ...
            if (ServiceLocator.Current.GetInstance<IEmpresaRules>().EsEscuela(idEscuela)) // validar si estoy logueado como escuela
            {
                var nivel =
                    ServiceLocator.Current.GetInstance<IEntidadesGeneralesRules>().GetNivelEducativoByEscuela(idEscuela)
                        .Id;
                if (nivel != (int) NivelEducativoNombreEnum.SUPERIOR)
                    // validar si estoy logueado como escuela de nivel superior
                {
                    if (ServiceLocator.Current.GetInstance<IEscuelaPlanRules>().TieneEscuelaPlan(idEscuela))
                    {
                        ...
                        ViewData.Add(ViewDataKey.GRADO_ANIO.ToString(),
                                     ServiceLocator.Current.GetInstance<IGradoAñoRules>().
                                         GetAllGradoAñoPorNivelEducativoDeEscuela(idEscuela));
                        return View();
                    }
                    TempData[AjaxAbmc.ErrorVista] =
                        "No se pueden realizar inscripciones ya que la escuela no tiene asignado un plan de estudio.";
                }
                else
                    TempData[AjaxAbmc.ErrorVista] =
                        "Opción válida unicamente para usuarios logueados como escuela de nivel inicial, primario o medio.";
                return RedirectToAction("Error", "Home");
            }
            TempData[AjaxAbmc.ErrorVista] = "Opción válida unicamente para usuarios logueados como escuela.";
            return RedirectToAction("Error", "Home");
        }
        ...
        public JsonResult ProcesarBusqueda(string sidx, string sord, int page, int rows, int? id, 
            ...
            DateTime? filtroPeriodoInscripcionDesde, DateTime? filtroPeriodoInscripcionHasta, int? filtroCicloLectivo, int? filtroEscuela)
        {
            ...
            var registros = Rule.GetByFiltros(filtroEscuela.HasValue ? filtroEscuela : idEscuela, filtroGradoAnio, filtroNroDocumentoEstudiante, filtroSexoEstudiante, filtroEspecialidad, filtroDivision, filtroTurno, filtroPeriodoInscripcionDesde, filtroPeriodoInscripcionHasta, filtroCicloLectivo);
       ...
       public JsonResult ObtenerDocumentosRequeridoPorProcesoGradoAnio(ProcesoEnum proceso, int gradoAnioId)
       {
        ...
        public JsonResult ObtenerDocumentosRequeridoPorProcesoPresentadosEstudiante(ProcesoEnum proceso, int idEstudiante, int idGradoAnio, int? filtroEscuela)
        {
            var rows = from DocumentoRequeridoModel e in ServiceLocator.Current.GetInstance<IEntidadesGeneralesRules>().GetDocumentosRequeridoPorProcesoPresentadosEstudiante(proceso, idEstudiante, filtroEscuela.HasValue ? filtroEscuela.Value : idEscuela, idGradoAnio, null)
                       select new { cell = new string[] { e.Id.ToString(), e.GradoAnio, e.Proceso.ToString(), e.Documento.Nombre } };
            return Json(new { rows = rows }, JsonRequestBehavior.AllowGet);
        }
       ...
       public JsonResult GetEstudianteByInscripcion(int idInscripcion)
       {
        ...
        public JsonResult GetTurnosByGradoAnio(int idGradoAnio, int? filtroEscuela)
        {
            var turnos = Rule.GetTurnosByGradoAnio(idGradoAnio, filtroEscuela.HasValue ? filtroEscuela.Value : idEscuela).OrderBy(x => x.Nombre);
            return Json(new SelectList(turnos, "Id", "Nombre"), JsonRequestBehavior.AllowGet);
        }

        public JsonResult GetEspecialidadesEscuelaAsociadasAGradoAnio(int idGradoAnio, int? filtroEscuela)
        {
            var especialidades = Rule.GetEspecialidadByGradoAño(idGradoAnio, filtroEscuela.HasValue ? filtroEscuela.Value : idEscuela).OrderBy(x => x.Nombre);
            return Json(new SelectList(especialidades, "Id", "Nombre"), JsonRequestBehavior.AllowGet);
        }

        public JsonResult GetDivisionesEscuelaByGradoAnioTurno(int idGradoAnio, int idTurno, int? filtroEscuela)
        {
            List<DivisionEnum> divisiones = Rule.GetDivisionesEscuelaByGradoAnioTurno(idGradoAnio, idTurno, filtroEscuela.HasValue ? filtroEscuela.Value : idEscuela);
        ...
        public JsonResult VerificarNivel(int? filtroEscuela)
        {
            var nivel = ServiceLocator.Current.GetInstance<IEntidadesGeneralesRules>().GetNivelEducativoByEscuela(filtroEscuela.HasValue ? filtroEscuela.Value : idEscuela).Id;
        ...
        public JsonResult MostrarLibroFolioInscripcionAnterior(int alumnoId)
        {
            ...
            return insc != null
                       ? Json(new {insc.LibroMatriz, insc.Folio}, JsonRequestBehavior.AllowGet)
                       : Json(new {}, JsonRequestBehavior.AllowGet);
        }

        public JsonResult GetGradoAñoByEscuela(int idEmpresa)
        {
            var grados = ServiceLocator.Current.GetInstance<IGradoAñoRules>().GetAllGradoAñoPorNivelEducativoDeEscuela(idEmpresa);
            return Json(new SelectList(grados, "Id", "Nombre"), JsonRequestBehavior.AllowGet);
        }
